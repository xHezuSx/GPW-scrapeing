...
def split_header(header) -> list:
    ...
    """
    header = header.split('|')[:-1]  # I do not know what is the last parameter, so I delete it
...
def get_file_name(report_id: int, file_id: int, company_name: str, file_title: str) -> str:
...
def scrape(company, limit, date, report_type, report_category, downlaod_csv, download_file_types):
    url_report_type = []
    ...
    for report in report_list:
        ...
        if report.find('p').text.strip() == '':  # instead of adding empty title we pass information about type report
        ...
        profit_change = report.find('span', attrs={'class': 'profit margin-left-30 pull-right'})
        loss_change = report.find('span', attrs={'class': 'loss margin-left-30 pull-right'})
        ...
        rate_change = report.find('span', attrs={'class': 'summary margin-left-30 pull-right'}).text.replace(',',
                                                                                                             '.').replace(
            'Kurs', '')
    ...
    if not os.path.exists('./REPORTS'):
        os.makedirs('./REPORTS')
    if not os.path.exists(f'./REPORTS/{company}') and (len(download_file_types) != 0 or downlaod_csv):
        if_downloaded = True
    ...
    if len(download_file_types) != 0:
        for i, link in enumerate(links):
            ...
            for j, (attachment, name) in enumerate(zip(attachments, file_titles)):
                filename = get_file_name(report_id=i+1, file_id=j+1, company_name=company, file_title=name)
    ...
    if downlaod_csv:
        ...
        if_downloaded = True
        output_info += f"| CSV file saved and file opened | "

    if if_downloaded:
        return f"SUCCESS! {output_info}\n files saved in:\n\t\t {current_path}\\REPORTS\\{company}", report_df

    return f"SUCCESS! {output_info}", report_df
